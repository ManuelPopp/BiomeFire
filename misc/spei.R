...
cmfl=list.files("/storage/brunp/Data/CHELSA_V.2.1/CMI/Monthly/",full.names=T)

#cmfl=cmfl[-c(grep("2017",cmfl),grep("2018",cmfl))]
yr=substr(basename(cmfl),start=15,stop=18)
mt=substr(basename(cmfl),start=12,stop=13)
...
for(i in 1:ncol(df.vals)){
  ...
  if(!all(is.na(df.vals[,i]))){
    ...
    spob.12=SPEI::spei(tsi,12,ref.start=c(1980,1), ref.end=c(2000,1))
    
    #spei.1[[i]]=as.numeric(spob.1$fitted)
    spei.12[[i]]=as.numeric(spob.12$fitted)
  } else {
    spei.12[[i]]=df.vals[,i]
  }
}

cbi.12=do.call("cbind",spei.12)
...
for(i in 1:nrow(cbi.12)){
  ...
  if(i>11){
    flo.12=paste0(sdirs[i],"/",gsub("cmi",paste0("spei.12_",id),basename(cmfl[i])))
