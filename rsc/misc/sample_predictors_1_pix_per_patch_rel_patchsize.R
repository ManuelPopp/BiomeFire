...
## In this version, we sample max 1 pixel per fire patch. The probability
## of sampling a fire pixel is proportional to the relative area of the
## patch.
##
#>----------------------------------------------------------------------------<|
...
if (!file.exists(f_full_vector) | recalculate) {
  ...
  full_vector$rel_area <- rel_areas

  terra::writeVector(full_vector, filename = f_full_vector, overwrite = TRUE)
} else {
  full_vector <- terra::vect(f_full_vector)
  rel_areas <- full_vector$rel_area
}

sample_list <- list()
weights <- rel_areas / sum(rel_areas)

## Sample n_samples fire locations
idx <- sample(1:nrow(full_xy_df), size = n_samples, prob = weights)
